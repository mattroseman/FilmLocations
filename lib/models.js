...
const { getCommonPrefix, getCoordinatesCenter } = require('../lib/utils.js');
 ...
 */
movieSchema.statics.getTopMovies = async function(movieIds=[], numMovies=100, offset=0)  {
  const aggregationPipeline = Movie.aggregate();
  ...
  movies = movies.map((movie) => {
    movie.locationsInfo = movie.locationsInfo.map((locationInfo) => {
      // match up the movie location descriptions with the locationInfo objects
      ...
      return {
        ...
        point: locationInfo.locationPoint,
        geohash: locationInfo.geohash,
...
locationSchema.statics.getClustersInBounds = async function(southWest, northEast, clusterFactor=3, movieId) {
  const cachePrefix = `clusters_${clusterFactor}_${movieId}`;

  const geohashes = geohash.bboxes(southWest[0], southWest[1], northEast[0], northEast[1], clusterFactor);
  let cachedGeohashes = new Set();
  let uncachedGeohashes = new Set();

  const cachedClusters = [];

  // check cache for each of these geohashes to see if they've already been calculated
  if (clusterFactor <= 5) {
    for await (const geohash of geohashes) {
      let cluster;

      try {
        cluster = await cacheGet(`${cachePrefix}_${geohash}`);
      } catch (err) {
        console.error(chalk.red(`something wen't wrong getting cached cluster for geohash: ${geohash}\n${err}`));
      }

      // if a cluster is found in clash, remove it from geohashes, and push it to the clusters array
      if (cluster) {
        // if the cached cluster isn't empty
        if (cluster !== '{}') {
          cachedClusters.push(JSON.parse(cluster));
        }

        cachedGeohashes.add(geohash);
        continue;
      }

      uncachedGeohashes.add(geohash);
    }

    if (uncachedGeohashes.size === 0) {
  ...
  if (clusterFactor <= 5) {
    aggregationPipeline.match({
      [`geohashPrefixes.${clusterFactor}`]: {
        $in: [...uncachedGeohashes]
      }
    });
  } else if (containingGeohashPrefix.length > 0) {
    // if there is a geohash prefix, filter by that
  ...
  aggregationPipeline.project({
    ...
    locationString: 1,
    movies: 1,
    groupingGeohashSubstr: `$geohashPrefixes.${clusterFactor}`
  });
  ...
  // cache the data if the cluster factor is less than or equal to 5
  if (clusterFactor <= 5) {
    // cache each of the returned clusters
    for await (const cluster of clusters) {
      try {
        await cacheSet(`${cachePrefix}_${cluster.id}`, JSON.stringify(cluster));
        cachedGeohashes.add(cluster.id);
      } catch (err) {
        console.error(chalk.red(`something wen't wrong saving cluster: ${cluster.id} to cache\n${err}`));
      }
    }

    // filter uncachedGeohashes to only those that aren't in cachedGeohashes
    uncachedGeohashes = new Set([...uncachedGeohashes].filter((uncachedGeohash) => {
      return !cachedGeohashes.has(uncachedGeohash);
    }));

    // cache each of the remain uncached geohashes (which should all be empty since no cluster data was returned)
    for (const uncachedGeohash of uncachedGeohashes) {
      try {
        await cacheSet(`${cachePrefix}_${uncachedGeohash}`, '{}');
      } catch (err) {
        console.error(chalk.red(`something wen't wrong saving geohash: ${uncachedGeohash} to cache\n${err}`));
      }
    }
  }

  // include the cached clusters with the returned clusters
  return [
    ...clusters,
    ...cachedClusters
  ];
...
locationSchema.statics.getTopMovies = async function(southWest, northEast, limit=100) {
  ...
  // TODO use geohashPrefixes like get clusters uses
  aggregationPipeline.match({
    'geohash': {
