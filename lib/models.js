 ...
 */
locationSchema.statics.getClustersInBounds = async function(southWest, northEast, clusterFactor=2) {
  const southWestGeohash = geohash.encode(southWest[0], southWest[1]);
  const northEastGeohash = geohash.encode(northEast[0], northEast[1]);
  const geohashCommonPrefix = getCommonPrefix(southWestGeohash, northEastGeohash);
  const geohashCommonLength = geohashCommonPrefix.length + clusterFactor;
  ...
  aggregationPipeline.project({
    ...
    movies: 1,
    filterGeohashSubstr: {
      $substr: ["$geohash", 0, geohashCommonPrefix.length]
    },
  ...
  */
  aggregationPipeline.match({
    filterGeohashSubstr: geohashCommonPrefix
  });
  ...
  return clusters.map((cluster) => {
    return {
      ...
      locations: cluster.points.map((point) => {
        if (point.coordinate === undefined || point.coordinate.length !== 2) {
          console.log(geohashCommonPrefix);
          console.log(point);
        }
        return {
